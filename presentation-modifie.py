...
if page == sections[5] :
    ...
    def prediction_temperature(country):
        # Créer une copie du dataset pour éviter les modifications
        df_work = dataset.copy()

        # Filtrer pour ne retenir que le pays choisi
        df_country = df_work[df_work['Name_EN'] == country].copy()

        # Créer une colonne de dates à partir de YEAR
        df_country['date'] = pd.to_datetime(df_country['YEAR'], format='%Y')

        # Passer la "date" en index
        df_country.set_index('date', inplace=True)

        # Ne garder que la colonne YAVGT (température)
        df_country = df_country[['YAVGT']]
        ...
        try:
            ...
            # Modèle Holt-Winters
            # Utiliser 'add' au lieu de 'mul' pour éviter les problèmes avec les valeurs négatives
            model_hw = ExponentialSmoothing(df_country['YAVGT'], trend='add', seasonal='add', seasonal_periods=5)
            hw_fit = model_hw.fit()
            ...
            # Prévisions SARIMAX - générer les dates futures correctement
            last_year = df_country.index[-1].year
            future_dates = pd.date_range(start=f'{last_year + 1}-01-01', periods=10, freq='YS')
            ...
            st.error(f"Erreur lors de la prédiction pour {country}: {str(e)}")
            st.info("Essayez un autre pays ou vérifiez que les données sont disponibles pour ce pays.")
    ...
    with st.container():
        ...
        st.image("./ressources/autocorrélation.png",
